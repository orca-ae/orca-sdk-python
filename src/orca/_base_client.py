...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def __init__(
        ...
        *,
        ...
        if max_retries is None:  # pyright: ignore[reportUnnecessaryComparison]
            raise TypeError(
                "max_retries cannot be None. If you want to disable retries, pass `0`; if you want unlimited retries, pass `math.inf` or a very high number; if you want the default behavior, pass `orca.DEFAULT_MAX_RETRIES`"
            )
    ...
    def _build_headers(self, options: FinalRequestOptions, *, retries_taken: int = 0) -> httpx2.Headers:
        ...
        headers, omitted = build_headers(
            {
                "x-orca-timeout": str(options.timeout.read)
                if isinstance(options.timeout, Timeout)
                else str(options.timeout),
                "x-orca-retry-count": str(retries_taken),
                **({"x-orca-read-timeout": str(read_timeout)} if read_timeout is not None else {}),
    ...
    def platform_headers(self) -> Dict[str, str]:
        return client_headers(self._version)
    ...
    def _should_retry_exception(self, err: BaseException) -> tuple[bool, httpx2.Response | None]:
        ...
        Only failures that opt into the retry policy by type are retried:
        `APIStatusError` (subject to the usual status-code policy) and
        `APIConnectionError` / `APITimeoutError`. The check walks each error's
        `__cause__` chain, so wrapping a retryable failure via `raise ... from err`
        preserves retries.
        """
        seen: set[int] = set()
    ...
    def _idempotency_key(self) -> str:
        return f"orca-python-retry-{uuid.uuid4()}"
...
class SyncAPIClient(BaseClient[httpx2.Client, Stream[Any]]):
    ...
    def request(
        ...
        *,
        ...
        for retries_taken in range(max_retries + 1):
            ...
            # the attempt produced an error-status response
    ...
    def _attempt_request(
        ...
        *,
        ...
        errors for the caller) happens above, so error responses stay inspectable
        first. Connection failures raise typed errors (`APITimeoutError`,
        `APIConnectionError`).
        """
...
def client_headers(version: str) -> Dict[str, str]:
    return {
        "X-Orca-Client": f"orca-sdk-python/{version}",
    }
...
# `x-orca-helper` is the one header whose values accumulate across layers into a single
# comma-separated, deduplicated value instead of the later layer's value replacing the
# earlier one.
_APPEND_HEADERS = frozenset({"x-orca-helper"})
...
def _append_header_value(existing: str, addition: str) -> str:
    ...
    Values are joined with `", "`, the same format the helper header builder uses
    when it builds the `x-orca-helper` header.
    """
