...
class Stream(Generic[_T]):
    ...
    def __stream__(self) -> Iterator[_T]:
        ...
        try:
            for sse in iterator:
                # This API does not constrain SSE event names: any well-formed `data:`
                # payload is yielded, and callers discriminate on the payload's own
                # `type` field. Only `ping` and `error` are handled specially below.
                if sse.event != "ping" and sse.event != "error":
                    try:
                        data = sse.json()
                    except Exception:
                        # A malformed `data:` line is skipped, never raised — a single bad
                        # frame must not tear down an otherwise healthy stream.
                        log.warning("Skipping SSE frame with non-JSON data: %r", sse.data)
                    else:
                        if is_dict(data) and "type" not in data and sse.event:
                            data["type"] = sse.event
